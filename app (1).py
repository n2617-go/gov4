...
def fetch_finmind_api(dataset, data_id, token, days=90):
    ...
    try:
        ...
        if res.status_code != 200: return pd.DataFrame(), f"HTTP {res.status_code}"
        result = res.json()
...
def analyze_stock(df, m_list, warn_p):
    ...
    for col in ['Close', 'High', 'Low', 'Open']:
        if col not in df.columns: return 50, [], None, None, {}, False
        df[col] = pd.to_numeric(df[col], errors='coerce')
    
    df = df.dropna(subset=['Close']).reset_index(drop=True)
    if len(df) < 20: return 50, [], df.iloc[-1], None, {"status":"資料不足", "reason":"需至少20日數據", "strategy":"觀望"}, False
    ...
    if len(matches) >= 3 and last['Close'] >= prev['Close']:
        status, reason, strategy = "五指標共振 (強多)", "趨勢極強，建議持股續抱。", "強力買進/續抱"
    elif last['RSI'] > 75 and last['Close'] > last['Upper']:
        status, reason, strategy = "指標高檔鈍化", "沿上軌噴發，建議設移動停利。", "警戒續抱"
    elif last['Close'] > df['Close'].tail(10).max() and last['OSC'] < df['OSC'].tail(10).max():
        status, reason, strategy = "背離訊號", "股價創高但 MACD 縮小，建議分批落袋。", "分批賣出"
    elif last['Close'] < last['MA20'] and prev['Close'] >= prev['MA20']:
        status, reason, strategy = "跌破關鍵支撐", "跌破布林中軌，短線趨勢轉空。", "果斷清倉"
    elif last['RSI'] < 30 and last['K'] > prev['K']:
        status, reason, strategy = "低檔打底完成", "底部背離且量能加溫，建議小量試單。", "小量試單"

    score = 50 + (len(matches) * 10) if last['Close'] >= prev['Close'] else 50 - (len(matches) * 5)
    return int(score), matches, last, prev, {"status": status, "reason": reason, "strategy": strategy}, is_warning
...
<style>
    ...
    .dec-box { background: #1e293b; padding: 12px; border-radius: 8px; margin: 12px 0; border: 1px solid #334155; }
    .warn-badge { background: #facc15; color: #000; padding: 2px 8px; border-radius: 4px; font-size: 0.7rem; font-weight: 800; position: absolute; top: 10px; right: 75px; }
</style>
...
if not st.session_state.auth:
    ...
    if st.button("驗證並開啟 AI 監控", use_container_width=True):
        check_df, _ = fetch_finmind_api("TaiwanStockInfo", None, t_input)
...
with st.sidebar:
    ...
    warn_p = st.slider("即時預警比例 (%)", 0.5, 10.0, 1.5)
...
for code in list(st.session_state.watchlist):
    ...
    if not df.empty and len(df) >= 2:
        score, matches, last, prev, dec, is_warn = analyze_stock(df, m_list, warn_p)
        ...
        # 決定策略顏色
        s_color = "#38bdf8"
        if "買" in dec['strategy'] or "續抱" in dec['strategy']: s_color = "#f87171"
        elif "賣" in dec['strategy'] or "清倉" in dec['strategy']: s_color = "#4ade80"

        # 核心修正：避免使用複雜嵌套，改用最穩定的字串拼裝
        html_str = f"""
        <div class="card" style="border-left-color: {color};">
            {"<div class='warn-badge'>⚠️ 波動達 " + str(warn_p) + "%</div>" if is_warn else ""}
            <div style="float:right; text-align:right;">
                ...
                <div style="margin-top:5px; font-weight:bold; color:{s_color}; font-size:0.8rem;">{dec['strategy']}</div>
            </div>
            <div style="font-size:1rem; font-weight:bold;">{name_map.get(code, code)} ({code})</div>
            <div style="font-size:1.8rem; font-weight:900; color:{color}; margin:8px 0;">{last['Close']:.2f} <span style="font-size:0.9rem;">({chg:+.2f}%)</span></div>
            
            <div class="dec-box">
                <div style="color:#94a3b8; font-size:0.75rem; font-weight:bold; margin-bottom:3px;">AI 決策說明：{dec['status']}</div>
                <div style="color:#f8fafc; font-size:0.85rem; line-height:1.4;">{dec['reason']}</div>
            </div>
            
            <div style="min-height:25px;">
                {" ".join([f'<span class="tag">{m}</span>' for m in matches]) if matches else '<span style="color:#475569; font-size:0.75rem;">目前無顯著指標訊號</span>'}
            </div>
        </div>
        """
        st.markdown(html_str, unsafe_allow_html=True)
        if st.button(f"🗑️ 移除 {code}", key=f"del_{code}"): st.session_state.watchlist.remove(code); st.rerun()
    else:
        st.warning(f"⚠️ {code}: 抓取失敗")

st.divider()
placeholder = st.empty()
for r in range(60, 0, -1):
    placeholder.caption(f"⏱️ 下次刷新倒數：{r} 秒")
    time.sleep(1)
st.rerun()
